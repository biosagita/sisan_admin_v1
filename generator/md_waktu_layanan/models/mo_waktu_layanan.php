...
class mo_waktu_layanan extends CI_Model {
	...
	function fnwaktu_layananCount() {
		...
		$vResult = $this->db->get(t_waktu_layanan)->result();
		if($vResult) {
			return $vResult[0];
		} else {
			return false;
	...
	function fnCreatewaktu_layanan($pData) {
		...
		$vResult = $this->db->insert('t_waktu_layanan',$vData);
		if($vResult) {
	...
	function fnwaktu_layananDelete($pDelwaktu_layananId) {
		...
		$vResult = $this->db->where('id_waktu_layanan',$pDelwaktu_layananId)->delete('t_waktu_layanan');
	...
	function fnUpdatewaktu_layanan($pwaktu_layananId,$pData) {
		...
		$vResult = $this->db->where('id_waktu_layanan',$pwaktu_layananId)->update('t_waktu_layanan',$vData);
