...
function fnDeleteRole_75() {
	var singleRow = $('#75_dtgUser').datagrid('getSelected');
	if (singleRow) {
		$.messager.confirm('Confirm','Are you sure you want to delete this data?',function(res) {
			if (res) {
				
				$.post('<?php echo base_url(); ?>index.php/md_user_role/fnuserDelete/'+singleRow.userrole_user_uid,function(result) {
				
					if (result.success) {
						$('#75_dtgUser').datagrid('reload');
					} else {
