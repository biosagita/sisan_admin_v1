					...
					<div style="padding:0px 15px;">
						...
						<div style="padding:10px;width:850px;border:0px dotted #CCCCCC;border-radius:5px;float:left;clear:left;">
								...
								<div class="frmItem">
									...
									<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-print" onclick="fnPrint_transaksi543_text('myPop1',1000,600)"><font color=#fff> Print Text</a>
...
function fnPrint_transaksi543_text( title,w,h) {
 		var startKeyword = $('#543startdate').datebox('getValue') || 'all';
		var finishKeyword = $('#543finishdate').datebox('getValue') || 'all';
		var LayananKeyword = $('#543layanan').val() || 'all';
		var LoketKeyword = $('#543loket').val() || 'all';
		var UserKeyword = $('#543user').val() || 'all';
			...
			var left = (screen.width/2)-(w/2);
			var top = (screen.height/2)-(h/2);
			var targetWin = window.open ('index.php/md_laporan_waktu_layanan/fntransaksiDataPrintText/'+startKeyword+'/'+finishKeyword+'/'+LayananKeyword+'/'+LoketKeyword+'/'+UserKeyword, title, 'toolbar=yes, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width='+w+', height='+h+', top='+top+', left='+left);
