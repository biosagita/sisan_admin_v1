...
//=========Skip List===========
	...
	function fnGoToNext($ticket_number) {
		...
		$cal_next=$this->db->query("Select id_transaksi,no_ticket,nama_layanan,waktu_ambil, b.id_group_layanan, b.id_layanan_forward from transaksi as a left join layanan as b ON a.id_layanan=b.id_layanan JOIN prioritas_layanan pl ON (a.id_group_layanan = pl.id_group_layanan) where status_transaksi='0' and a.id_group_layanan IN (".join(',', $listlayanan).") and tanggal_transaksi='$tanggal_transaksi' and no_ticket_awal='$no_ticket_awal' and no_ticket='$no_ticket' order by pl.Prioritas ASC, id_transaksi asc LIMIT 1");
		...
		if(!empty($vRow_next['id_transaksi'])) {
			...
			$sql=$this->db->query("UPDATE transaksi set  status_transaksi='1',waktu_panggil='$waktu_panggil',id_loket='$Loket' where no_ticket='$next_id' and id_group_layanan IN (".join(',', $listlayanan).") and tanggal_transaksi='$tanggal_transaksi'");

			if ($_countmk3['tanggal_transaksi'] > '0')
			{
				$sql=$this->db->query("UPDATE transaksi set waktu_finish='$waktu_finish', status_transaksi='5', id_user='".$this->session->userdata('sId')."' where no_ticket='$_countmk3[no_ticket]' and id_loket='$Loket' and tanggal_transaksi='$tanggal_transaksi' ");
	...
	$cal_next=$this->db->query("Select id_transaksi,no_ticket,nama_layanan,waktu_ambil, b.id_group_layanan, b.id_layanan_forward from transaksi as a left join layanan as b ON a.id_layanan=b.id_layanan JOIN prioritas_layanan pl ON (a.id_group_layanan = pl.id_group_layanan) where status_transaksi='0' and a.id_group_layanan IN (".join(',', $listlayanan).") and tanggal_transaksi='$tanggal_transaksi'order by pl.Prioritas ASC, id_transaksi asc  ");
	$vRow_next = $cal_next->row_array(); 
	$id_transaksi=$vRow_next['id_transaksi'];
	$next_id=$vRow_next['no_ticket'];	
	$transaction=$vRow_next['nama_layanan'];
	...
	function fnforward($id_layanan, $id_group_layanan) {
		...
		$q = 'SELECT gl.nama_group_layanan, tr.id_transaksi, tr.no_ticket, tr.waktu_ambil, tr.no_ticket_awal, tr.id_layanan, tr.id_group_layanan FROM transaksi tr JOIN group_layanan gl ON (tr.id_group_layanan = gl.id_group_layanan) JOIN prioritas_layanan pl ON (tr.id_group_layanan = pl.id_group_layanan) WHERE pl.id_group_loket = ('.$grouploket.') AND tr.id_group_layanan IN ('.join(',', $listlayanan).') AND tr.status_transaksi IN (1,2) AND tr.tanggal_transaksi = "'.$currentDate.'" ORDER BY pl.Prioritas ASC, tr.id_transaksi LIMIT 1';
		$query = $this->db->query($q);
