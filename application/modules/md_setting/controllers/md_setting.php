...
class md_setting extends MX_Controller {
	...
	function fnsetting()	{
		...
		$vData['data_row'] = $this->mo_setting->fnsettingRow_New();		
	...
	function fnUpdatesetting_New() {
		if(!empty($_POST)) {
			$arrData = array();
			foreach ($_POST as $key => $value) {
				$id_setting = str_replace('own_input_', '', $key);
				$arrData[] = array(
					'id_setting' 	=> $id_setting,
					'nilai' 		=> $value,
				);
			}
			$this->mo_setting->fnUpdatesetting_New($arrData);
		}
	}
}
?>
