...
class mo_setting extends CI_Model {
	...
	function fnsettingData($psettingKeyword,$pOffset,$pRows,$pSort,$pOrder) {
		...
		foreach($vResult as $vRow):
			...
			$vArrayTemp['type_input'] = $vRow->type_input;

			$vArrayTemp['refer_table'] = $vRow->refer_table;

			if(!empty($vArrayTemp['refer_table'])) {
				$vArrayTemp['option_refer_table'] = getPilihanReferTable($vArrayTemp['refer_table']);
			}		
	...
	function fnsettingRow_New() {		
		...
		foreach ($vResult as $key => $value) {
			$vArrayTemp[$value->keterangan][$value->id_setting] = (array) $value;
			$refertable = $value->refer_table;
			if(!empty($refertable)) {
				$vArrayTemp[$value->keterangan][$value->id_setting]['option_refer_table'] = $this->getPilihanReferTable($refertable);
			}
		}
	...
	function fnUpdatesetting_New($arrData) {
		if(!empty($arrData)) {
			foreach ($arrData as $key => $value) {
				$vData = array(		
					'nilai'=>$value['nilai'],					
				);	
				$vResult = $this->db->where('id_setting',$value['id_setting'])->update('setting',$vData);	
			}
		}
		echo json_encode(array('success'=>true));
	}

	function getPilihanReferTable($namaTable) {
		$vResult = $this->db->get($namaTable)->result();
		$vItems = array();
		foreach($vResult as $vRow) {
			$vItems[$vRow->nilai_refer] = $vRow->nilai_refer;
		}
		return $vItems;
	}
