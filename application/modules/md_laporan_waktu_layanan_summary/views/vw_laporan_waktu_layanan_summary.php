					...
					<div style="padding:0px 15px;">
						...
						<div style="padding:10px;width:850px;border:0px dotted #CCCCCC;border-radius:5px;float:left;clear:left;">
								...
								<div class="frmItem" style="margin-bottom:5px;">
									...
									<select style="display:inline-block;width:380px;" id="5443layanan" name="5443layanan">
								...
								<div class="frmItem" style="margin-bottom:5px;">
									...
									<select style="display:inline-block;width:380px;" id="5443loket" name="5443loket">
								...
								<div class="frmItem" style="margin-bottom:5px;">
									...
									<select style="display:inline-block;width:380px;" id="5443user" name="5443user">
								...
								<div class="frmItem">
									...
									<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-print" onclick="fnPrint_transaksi5443_text('myPop1',1000,600)"><font color=#fff> Print Text</a>
									...
									<a href="javascript:void(0)" class="easyui-linkbutton"  onclick="fnPrint_rekap_chart_pertanyaan('myPop1',1000,600)"><font color=#fff>Chart</a>
								</div>
...
function fnPreview5443(){
	$('#5443_dtglaporan').datagrid('load',{
		...
		LayananKeyword: $('#5443layanan').val(),
		LoketKeyword: $('#5443loket').val(),
		UserKeyword: $('#5443user').val(),
...
function fnPrint_transaksi5443( title,w,h) {
		...
		var LayananKeyword = $('#5443layanan').val() || 'all';
		var LoketKeyword = $('#5443loket').val() || 'all';
		var UserKeyword = $('#5443user').val() || 'all';
 
			var left = (screen.width/2)-(w/2);
			var top = (screen.height/2)-(h/2);
			var targetWin = window.open ('index.php/md_laporan_waktu_layanan_summary/fntransaksiDataPrint/'+startKeyword+'/'+finishKeyword+'/'+LayananKeyword+'/'+LoketKeyword+'/'+UserKeyword+'/'+PilihanKeyword+'/'+UnitTimeKeyword, title, 'toolbar=yes, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width='+w+', height='+h+', top='+top+', left='+left);
		
}

function fnPrint_transaksi5443_text( title,w,h) {
 		var startKeyword = $('#5443startdate').datebox('getValue') || 'all';
		var finishKeyword = $('#5443finishdate').datebox('getValue') || 'all';
		var PilihanKeyword = $('input[name="5443rdpilihan"]:checked').val() || 'all';
		var UnitTimeKeyword = $('#5443unittime').val() || 'all';
		...
		var UserKeyword = $('#5443user').val() || 'all';
			...
			var left = (screen.width/2)-(w/2);
			var top = (screen.height/2)-(h/2);
			var targetWin = window.open ('index.php/md_laporan_waktu_layanan_summary/fntransaksiDataPrintText/'+startKeyword+'/'+finishKeyword+'/'+LayananKeyword+'/'+LoketKeyword+'/'+UserKeyword+'/'+PilihanKeyword+'/'+UnitTimeKeyword, title, 'toolbar=yes, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width='+w+', height='+h+', top='+top+', left='+left);
		
}

function fnPrint_rekap_chart_pertanyaan( title,w,h) {
		var startKeyword = $('#5443startdate').datebox('getValue') || 'all';
		var finishKeyword = $('#5443finishdate').datebox('getValue') || 'all';
		var PilihanKeyword = $('input[name="5443rdpilihan"]:checked').val() || 'all';
		var UnitTimeKeyword = $('#5443unittime').val() || 'all';
		...
		var UserKeyword = $('#5443user').val() || 'all';
			...
			var left = (screen.width/2)-(w/2);
			var top = (screen.height/2)-(h/2);
			var targetWin = window.open ('index.php/md_laporan_waktu_layanan_summary/fntransaksiDataPrintChart/'+startKeyword+'/'+finishKeyword+'/'+LayananKeyword+'/'+LoketKeyword+'/'+UserKeyword+'/'+PilihanKeyword+'/'+UnitTimeKeyword, title, 'toolbar=yes, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width='+w+', height='+h+', top='+top+', left='+left);
